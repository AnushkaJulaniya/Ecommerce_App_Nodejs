...
const createOrder = async(req, res) => {
     ...
     * 3. Check  for coupon
     * 3.1 If not applied proceed to step 3
     * 3.2 If applied , validate the coupon and apply the discount
     * 4. Check payment method
     * 4.1 If Cash on Delivery / Payment on Delivery proceed to step 5
...
const userCart = await CartModel.findOne({userId:req.user._id}, { products:1, _id:0})
...
const productsAvaliable = userCart.products.every(p => p.productId && p.productId.stock >= p.qty);
